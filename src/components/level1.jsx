...
export default function Level1() {
  ...
  const [score, setScore] = useState(() => {
    ...
    const saved = localStorage.getItem("level1_score");
  ...
  const [remainingLives, setRemainingLives] = useState(() => {
    ...
    const saved = localStorage.getItem("level1_lives");
  ...
  // Load progress from MongoDB only if we don't have values from navigation state
  useEffect(() => {
  ...
  const handleNextPosition = async () => {
    ...
    if (currentPosition < levelData.puzzles.questions.length - 1) {
      ...
      if (COORDINATES[nextPosition]) {
        ...
        localStorage.setItem("level1_score", finalScore); // Store updated score
        localStorage.setItem("level1_lives", remainingLives); // Store remaining lives
      }
    } else {
      try {
        const finalScore = score + scoringData.levelCompletion;
        ...
        completionMessage.querySelector("button").addEventListener("click", () => {
          ...
          localStorage.setItem("level1_currentPosition", COORDINATES[0]); // Reset to the initial position
      ...
      <div className="level1-header">
        ...
        <div className="header-right">
          <div className="score-display">
            <img src="/assets/dollar.png" className="coin-image" alt="Score" />
            {score}
          </div>
